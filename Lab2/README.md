...
9. This job will run for roughly around 3 minutes.
...
10. For Choose an IAM role, select Choose an existing IAM role, and in the drop-down pick the role made in the previous section, e.g. `AWSGlueServiceRole-<username>-crawler-role`, and click on **Next**.

11. In Create a schedule for this crawler, pick frequency as **Run on demand** and click on **Next**.

12. For Configure the crawler's output, under **Database**, click the drop-down and selerct the database used for Lab 1 and Lab 2, i.e. `<username>`. For **Prefix added to tables**, enter `taxi_` and click **Next**. 
